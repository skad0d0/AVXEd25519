...
void conv_char_to_NAF(NAFResult *r, const signed char *r_signed)
{
    ...
    for (int i = 255; i >= 0; i--) {
        // Convert signed char to uint32_t using two's complement
        r->k[i] = (uint32_t)((int32_t)r_signed[i]);

        // If the first non-zero element is found, set the length
        if (r->length == 0 && r->k[i] != 0) {
            r->length = i + 1;  // Length is index + 1
            // Continue converting the rest of the array
        }
    }
}
...
void sc25519_slide(signed char *r_signed, const uint32_t *s, int swindowsize) {
    int i, j, k, b;
    signed char m = (1 << (swindowsize - 1)) - 1; // Maximum coefficient value within window
    const int soplen = 256;

    /* Extract the scalar's components */
    uint32_t sv0 = s[0];
    uint32_t sv1 = s[1];
    uint32_t sv2 = s[2];
    uint32_t sv3 = s[3];
    ...
    /* Step 1: Convert the scalar to its binary representation */
    for (i = 0; i < 32; i++) {
        r_signed[i]       = (sv0 & 1);
        r_signed[i + 32]  = (sv1 & 1);
        r_signed[i + 64]  = (sv2 & 1);
    ...
    /* Step 2: Apply the sliding window technique */
    for (j = 0; j < soplen; ++j) {
        if (r_signed[j] != 0) {
            for (b = 1; b < soplen - j && b <= swindowsize; ++b) {
                /* Attempt to combine bits within the window */
                if (r_signed[j] + (r_signed[j + b] << b) <= m) {
                    r_signed[j] += r_signed[j + b] << b;
                    r_signed[j + b] = 0;
                }
                /* Attempt to subtract bits within the window */
                ...
                /* If the combination isn't possible, stop expanding the window */
